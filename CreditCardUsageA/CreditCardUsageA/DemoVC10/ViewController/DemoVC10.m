...
- (void)viewDidLoad {
    ...
    UIBarButtonItem *rightBarButtonItem = [[UIBarButtonItem alloc] initWithTitle:@"日" style:UIBarButtonItemStyleDone target:self action:@selector(rightBarButtonItemAction:)];
...
//        // 字典写入文件
//        // 创建一个存储字典的文件路径
//        NSString *fileDicPath = [docPath stringByAppendingPathComponent:@"data.plist"];
//        
//        // 字典写入时执行的方法
//        [dict writeToFile:fileDicPath atomically:YES];
        
        NSString *fileDicPath = [[NSBundle mainBundle]pathForResource:@"data01" ofType:@"plist"];
        
        NSMutableDictionary *dict = [[NSMutableDictionary alloc] initWithContentsOfFile:fileDicPath];
        ...
        NSString *json = [XYString getJsonStringFromObject:dict];
        NSLog(@"json = %@",json);
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    detailViewController.model = threeModel;
